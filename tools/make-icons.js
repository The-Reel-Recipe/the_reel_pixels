...
/* ── Drawn here, because the library has none ─────────────────────
   pixelarticons ships no eraser, and `delete` (the backspace key) was
   reading as "go back" next to a hand and a brush. This is the only
   hand-drawn glyph in the set, so it is held to the library's grammar:
   the same 24×24 box, the same 2px block, outline weight, no curves.

   A tilted rubber, seen corner-on, with the band across its worn end. */
const DRAWN = {
  eraser: [
    /* the outline, corner-stepped the way the library draws its own
       diagonals (see `check`): top vertex, down both sides, bottom vertex */
    [12, 2], [10, 4], [14, 4], [8, 6], [16, 6], [6, 8], [18, 8],
    [4, 10], [20, 10], [6, 12], [18, 12], [8, 14], [16, 14],
    [10, 16], [14, 16], [12, 18],
    /* the band, parallel to the sides, cutting off the worn end */
    [8, 10], [10, 12], [12, 14]
  ]
};

/* one 2×2 block per pair, in the compact form the library itself uses */
const blocks = pts => pts.map(([x, y]) => `M${x} ${y}h2v2H${x}z`).join('');
...
function sprite() {
  ...
  for (const [ours, pts] of Object.entries(DRAWN)) {
    symbols.push(`      <symbol id="i-${ours}" viewBox="0 0 24 24"><path d="${blocks(pts)}"/></symbol>`);
  }
  return [
    START,
